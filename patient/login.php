...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        $sql = "SELECT pl.*, p.first_name, p.last_name, p.date_of_birth, p.gender 
                FROM patient_login pl 
                JOIN patients p ON pl.ssn = p.ssn 
                WHERE pl.username = ? AND pl.is_active = 1";
        $patient = $db->fetchOne($sql, [$username]);
        
        if ($patient && password_verify($password, $patient['password'])) {
            // Update last login
            $db->query("UPDATE patient_login SET last_login = NOW() WHERE id = ?", [$patient['id']]);
            
            // Set session
            $_SESSION['patient_id'] = $patient['id'];
            $_SESSION['patient_ssn'] = $patient['ssn'];
            ...
            display_message('Welcome back, ' . $patient['first_name'] . '!', 'success');
            redirect('dashboard.php');
        } else {
