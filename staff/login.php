...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        $sql = "SELECT sl.*, ms.first_name, ms.last_name, ms.position, ms.department, ms.hospital_id, h.name as hospital_name 
                FROM staff_login sl 
                JOIN medical_staff ms ON sl.ssn = ms.ssn 
                LEFT JOIN hospitals h ON ms.hospital_id = h.id 
                WHERE sl.username = ? AND sl.is_active = 1";
        $staff = $db->fetchOne($sql, [$username]);
        
        if ($staff && password_verify($password, $staff['password'])) {
            // Update last login
            $db->query("UPDATE staff_login SET last_login = NOW() WHERE id = ?", [$staff['id']]);
            
            // Set session
            $_SESSION['staff_id'] = $staff['id'];
            $_SESSION['staff_ssn'] = $staff['ssn'];
            ...
            $_SESSION['hospital_name'] = $staff['hospital_name'];
            $_SESSION['user_type'] = 'staff';
            
            display_message('Welcome back, ' . $staff['first_name'] . '!', 'success');
            redirect('dashboard.php');
        } else {
